...
def create_fakenewsdetclf_table():
    c.execute('CREATE TABLE IF NOT EXISTS fakenewsdetclfTable(rawtext TEXT,prediction TEXT,probability NUMBER,timeOfvisit TIMESTAMP)')

def add_prediction_details(rawtext,prediction,probability,timeOfvisit):
    c.execute('INSERT INTO fakenewsdetclfTable(rawtext,prediction,probability,timeOfvisit) VALUES(?,?,?,?)',(rawtext,prediction,probability,timeOfvisit))
    conn.commit()

def view_all_prediction_details():
    c.execute('SELECT * FROM fakenewsdetclfTable')
    data = c.fetchall()
    return data